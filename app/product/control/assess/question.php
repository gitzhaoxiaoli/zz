...
if( $cti_code=trim($cti_code) ){
	...
	$resdb = array();
    $sql = "SELECT $fields FROM `sp_assess_notes` a $join where 1 $where and iso_prod_type = 1 ORDER BY a.id DESC $pages[limit]";

	$query = $db->query( $sql );
	while( $rt = $db->fetch_array( $query ) ){
		$rt[ep_name]=$db->get_var("SELECT ep_name FROM `sp_enterprises` WHERE `eid` = '$rt[eid]'");
		$rt[name]=$db->get_var("SELECT name FROM sp_task_audit_team WHERE `role` = '1001' and tid='{$rt[tid]}' and deleted=0");
		//$rt[tb_date] = $db->get_var("SELECT name FROM sp_task_audit_team WHERE `role` = '1001' and pid='{$rt[pid]}'");
		//$rt[name]=$db->get_var("SELECT name FROM sp_task_audit_team WHERE `role` = '1001' and pid='{$rt[pid]}'");
