...
if( $audit_type_array ){
	foreach( $audit_type_array as $code => $item ){
		if( !in_array( $code, array( '1002', '1003','1007','1008' ) ) )
...
//标签样式
	...
	$redata_0 = $redata_1 = '';
	${'redata_'.$status} = " ui-tabs-active ui-state-active";
...
if( $cti_code=trim($cti_code) ){
	...
	//要获取的字段
	$fields .= "p.*,t.upload_file_date,t.upload_plan_date,t.bufuhe,p.ctfrom,p.audit_type,p.audit_ver,p.iso,p.redata_note,p.eid,p.tid,p.ct_id";
	...
	//检验不用收资料
	$where .= " AND p.deleted = '0' AND  p.status = 3 AND p.redata_status <> 0 AND p.iso_prod_type = 1";
    //$where .= " AND t.te_date < '".date("Y-m-d H:i:s")."'";

	if( !$export ){
		$restate_total = array(0,0);
		$restate_total[0]=$db->get_var("SELECT COUNT(*) total FROM sp_project p $join WHERE 1 $where AND p.assess_review_status =0 ");
		
		$restate_total[1]=$db->get_var("SELECT COUNT(*) total FROM sp_project p $join WHERE 1 $where AND p.assess_review_status =1 ");
		/*
		$query = $db->query("SELECT p.status,COUNT(*) total FROM sp_project p WHERE 1 $where GROUP BY p.status");
		
		while( $rt = $db->fetch_array( $query ) ){
			$restate_total[$rt['status']] = $rt['total'];
		}
		*/
		 $pages= numfpage( $restate_total[$status], 20 );
	}
	$where .= " AND p.assess_review_status = '$status'";
