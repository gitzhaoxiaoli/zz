...
//生产者名称
if($ep_manu_id){
    $res = $db->getField("enterprises","eid",array('ep_name'=>$ep_manu_id));
    $where .=" AND p.ep_manu_id = $res";
}
//生产企业名称
if($ep_prod_id){
    $res = $db->getField("enterprises","eid",array('ep_name'=>$ep_prod_id));
    $where .=" AND p.ep_prod_id = $res";
}
//省份
...
if( $cti_code=trim($cti_code) ){ //合同项目编码
	$cti_ids=array(-1);
	$query = $db->query("SELECT cti_id FROM sp_contract_item WHERE cti_code like '%$cti_code%' and deleted=0 and iso_prod_type = 1");
	...
	$ne_total = array(
	'1' => $db->get_var("SELECT COUNT(*) total FROM sp_project p $join WHERE 1 $where AND p.ifchangecert = '1' and p.iso_prod_type = 1"),
	'2' => $db->get_var("SELECT COUNT(*) total FROM sp_project p $join WHERE 1 $where AND p.ifchangecert = '2' and p.iso_prod_type = 1")
	);

	$pages = numfpage( $ne_total[$is_check] );
	$where .= "  and p.ifchangecert ='$is_check'";
	$sql = "SELECT p.*,e.ep_name FROM sp_project p $join WHERE 1 $where and p.iso_prod_type = 1 ORDER BY p.id DESC $pages[limit]";
	$query = $db->query( $sql );
	...
	while( $rt = $db->fetch_array( $query ) ){
        ...
        $rt[ep_manu_id] = $db->getField("enterprises",'ep_name',array('eid'=>$rt[ep_manu_id]));
        $rt[ep_prod_id] = $db->getField("enterprises",'ep_name',array('eid'=>$rt[ep_prod_id]));
		$datas[] = $rt;
	}
	if( !$export ){
		tpl( 'certificate/alist' );
	} else { //导出excel表格
		ob_start();
		tpl( 'xls/alist_certificate' );
