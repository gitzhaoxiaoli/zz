...
if( $cti_code=trim($cti_code) ){
  ...
  if($fac_code=trim($fac_code)){
	...
	if( !$export ){
		$status_total[0] = $db->get_var("SELECT COUNT(*) FROM sp_sample ss $join WHERE 1 $where AND ss.test_id = 0 ORDER BY t.te_date DESC");
		$status_total[1] = $db->get_var("SELECT COUNT(*) FROM sp_sample ss $join WHERE 1 $where AND ss.test_id <> 0 ORDER BY t.te_date DESC");
		$pages = numfpage( $status_total[$status] );
	}

	if($status)
		$where .= " AND ss.test_id <> 0";
	else
		$where .= " AND ss.test_id = 0";
