...
if( $cti_code=trim($cti_code) ){
  ...
  //生产企业搜索
	...
	$where .= " AND p.deleted=0 AND p.iso_prod_type = 1 AND is_samp = 1 AND is_qualified = 1";
	if( !$export ){
		$query = $db->query("SELECT p.test_status,COUNT(*) total FROM sp_project p WHERE 1 $where GROUP BY p.test_status");
		while( $rt = $db->fetch_array( $query ) ){
			$pd_type_total[$rt['test_status']] = $rt['total'];
		}
		$pages = numfpage( $pd_type_total[$pd_type] );
	}

	$where .= " AND p.test_status = $pd_type";
