...
$fields = $join = $where = $page_str = '';
	...
	//echo $where .= " AND e.fac_code != ''";// 只显示生产企业

	$total = $db->get_var("SELECT COUNT(*) FROM sp_enterprises e $join WHERE 1 AND e.deleted = '0' $where");
	$pages = numfpage( $total, 20 );

	$enterprises = array();
	$sql = "SELECT e.*,hr.name input_user FROM sp_enterprises e $join WHERE 1 AND e.deleted = '0' $where ORDER BY e.fac_code DESC $pages[limit]";
	$query = $db->query( $sql );
