...
$contracts = array();
$app_type_array = array("初次申请","变更申请","转换申请");
extract($_GET);
//搜索
if($cti_code){
...
$total = array();
$total[0] = $db->find_num("contract_item","$where and iso_prod_type = 1 and deleted = 0 and status = 0");
$total[1] = $db->find_num("contract_item","$where and iso_prod_type = 1 and deleted = 0 and status = 1");
$total[2] = $db->find_num("contract_item","$where and iso_prod_type = 1 and deleted = 0 and status = 2");
/* $total[3] = $db->find_num("contract_item","$where and iso_prod_type = 1 and deleted = 0");
 */
