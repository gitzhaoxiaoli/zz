...
if( $cti_code ){ //批次项目编码
	$where .= " AND cti.cti_code = '$cti_code'";
}
...
$pages = numfpage( $state_total[$is_bao], 20, "?app=cqm&m=$m&a=$a&is_bao=$is_bao" );

$where .= " and p.is_bao='$is_bao'  ";

$task_auditLists = array();
...
if( $task_auditLists ){
	...
	$team_where = " AND tat.pid IN (" . implode(',' , array_keys( $task_auditLists ) ) . ")";
...
//输出Execl文件
$filename = iconv( 'UTF-8', 'GB2312', '检查计划上报_').mysql2date( "Y-m-d", nowTime( 'mysql' ) ).".xls";
...
template( 'xls/list_plan_project' );
