...
$iso			= getgp( 'iso' ); //认证体系
$audit_start_start	= getgp('audit_start_start'); //检查开始时间 起
$audit_start_end= getgp('audit_start_end'); //检查开始时间 止
$audit_end_start= getgp('audit_end_start'); //检查结束时间 起
$audit_end_end	= getgp('audit_end_end'); //检查结束时间 止
...
$task_auditLists = $tids = array();
$sql = "SELECT $fields FROM sp_task t $join WHERE 1 $where ORDER BY t.id DESC" ;

$query = $db->query( $sql);
while( $rt = $db->fetch_array( $query ) ){
	isset( $task_auditLists[$rt['tid']] ) or $task_auditLists[$rt['tid']] = array(
			'ep_name'	=> $rt['ep_name'],
	...
	isset( $task_auditLists[$rt['tid']]['audit_vers'] ) or $task_auditLists[$rt['tid']]['audit_vers'] = array();
	isset( $task_auditLists[$rt['tid']]['audit_types'] ) or $task_auditLists[$rt['tid']]['audit_types'] = array();
	isset( $task_auditLists[$rt['tid']]['cti_codes'] ) or $task_auditLists[$rt['tid']]['cti_codes'] = array();

	$task_auditLists[$rt['tid']]['audit_vers'][] = f_audit_ver( $rt['audit_ver'] );
	$task_auditLists[$rt['tid']]['audit_types'][] = read_cache('audit_type', $rt['audit_type'] );
	$task_auditLists[$rt['tid']]['cti_codes'][] = $rt['cti_code'];


	$tids[] = $rt['tid'];
	//$task_auditLists[$rt['tid']] = $rt;
}
...
if( $task_auditLists ){
	...
	while( $rt = $db->fetch_array( $query ) ){
		...
		$str = '';
		$str .= $rt['name'];
		$str .= "(".read_cache('iso',$rt['iso']).":".read_cache('qualification',$rt['qua_type']).")";
...
//输出Execl文件
$filename = iconv( 'UTF-8', 'GB2312', $task_status[$status].'计划_').mysql2date( "Y-m-d", nowTime( 'mysql' ) ).".xls";
