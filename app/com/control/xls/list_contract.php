...
if( $contracts ){
	...
	while( $rt = $db->fetch_array( $query ) ){
		...
		$contracts[$rt['ct_id']]['audit_types'][] = read_cache('iso',$rt['iso']) . "：" . read_cache('audit_type', $rt['audit_type'] );
		$contracts[$rt['ct_id']]['audit_codes'][] = read_cache('iso',$rt['iso']) . "：" . $rt['audit_code'];
...
//输出Execl文件
$filename = iconv( 'UTF-8', 'GB2312', $state_V[$status].'批次_').mysql2date( "Y-m-d", nowTime( 'mysql' ) ).".xls";
