...
$arr['a1'] = get_conf('zdep_id');
$arr['a2'] = get_conf('zdep_id');
...
FROM sp_project p LEFT JOIN sp_enterprises e on e.eid=a.eid    WHERE a.deleted='0' and a.assess_status='1' and a.audit_type IN ('1004-1','1004-2','1004-3') and a.assess_date >= '$begindate' and a.assess_date <= '$enddate' group by a.id ORDER BY a.assess_date DESC";
$res       = $db->query($sql);
while ($row = $db->fetch_array($res)) {
    ...
    $arr['a13'] = $db->get_var("select meta_name from sp_metas where used='enterprise' and meta_name='ep_phone' and ID='$row[eid]' "); //13.组织联系电话
    $arr['a14'] = $db->get_var("select meta_name from sp_metas where used='enterprise' and meta_name='ep_fax' and ID='$row[eid]' "); //14.组织联系传真
    $arr['a15'] = $row['delegate']; //15.组织法定代表人
    $arr['a16'] = $row['nature']; //16.机构类型代码
    $arr['a17'] = $row['capital']; //17.组织注册资本
    $arr['a18'] = $row['currency']; //18.组织注册资本币种
    $arr['a19'] = $row['ep_amount']; //19.组织组织人数
    $arr['a20'] = $cti_info['total']; //20.与产品有关人数
    $arr['a21'] = $zs_info['first_date']; //21.初次获证日期
    if (!$arr['a21'])
        $arr['a21'] = $zs_info['s_date'];
    ...
    $arr['a31'] = ''; //31.变更类型代码(审定)
    $arr['a32'] = ''; //32.变更时所处的检查活动期间
    $arr['a33'] = ''; //33.书否换证
    $arr['a34'] = ''; //34.换证原因
    $arr['a35'] = ''; //35.原证书注册号
    $arr['a36'] = ''; //36.原证书颁发机构
    ...
    if ($arr['a32'] == '0301' || $arr['a32'] == '0302') {
        if ($row['audit_type'] == '1004-1') {
            ...
            $arr['a39'] = '2'; //39.监督次数
        } elseif ($row['audit_type'] == '1004-3') {
            $arr['a39'] = '3'; //39.监督次数
        }
    }
    $arr['a40'] = substr($task_info['tb_date'], 0, 10); //40.检查开始日期
    $arr['a41'] = substr($task_info['te_date'], 0, 10); //41.检查结束日期
    $arr['a42'] = $row['st_num']; //42.检查人日
    ...
    $sql        = "select invoice,invoice_cost ,currency from sp_cost_detail where id='$row[ccd_id]' ";
...
while ($row = $db->fetch_array($res)) {
    ...
    */
    ...
    $arr['a44'] = "$pd_row[comment_a_name]；$pd_row[comment_b_name]"; //审定人员
    $sql        = "select invoice,invoice_cost ,currency from sp_cost_detail where id='$row[ccd_id]' ";
    $cw_info    = $db->get_row($sql);
    $arr['a45'] = $cw_info['invoice_cost']; //收费金额
    $arr['a46'] = $cw_info['currency']; //收费币种
    $arr['a47'] = $cw_info['invoice']; //收费发票号
    $sql        = "select meta_value from sp_metas where used = 'contract_item' and meta_name = 'risk_level' and ID='$row[cti_id]' ";
    $arr['a48'] = $db->get_var($sql); //风险系数
    $arr['a49'] = $zs_info['s_date']; //证书发证日期
    $arr['a50'] = $zs_info['e_date']; //证书到期日期
    $arr['a51'] = sprintf('%02d', $zs_info['status']); //证书状态
...
while ($rt = $db->fetch_array($query)) {
    $auditor    = array(
        ...
        'a10' => $rt['qua_no'],
        'a11' => ($rt['is_leader'] ? '01' : ('1004-1' == $rt['role'] ? '03' : '02')),
...
$target_db   = DATA_DIR . 'access/' . mysql2date('Y-m-d', nowTime('mysql')) . '.mdb';
...
foreach ($data_arr as $key => $item) {
    ...
    load('data')->insert($sql, $target_db);
}
...
echo strstr($target_db, 'data');
exit;
//输出Execl文件
/**/
$filename = iconv('UTF-8', 'GB2312', '获证组织基本信息表_') . mysql2date("Y-m-d", nowTime('mysql')) . ".xls";
