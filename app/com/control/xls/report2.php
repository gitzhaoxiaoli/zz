...
//print_rr($data_arr);
//审定 （审定通过，监督类型,非换证(又取消 认监委的换证指的是证书号码变化)）
//and a.if_cert>'y'
$sql = "SELECT p.id,a.ct_id,a.cti_id
FROM sp_assess a inner join sp_enterprises e on e.eid=a.eid left join sp_project p on p.id=a.pid WHERE a.deleted='0' and a.assess_status='1' and (a.audit_type='1004-1' or a.audit_type='1004-2' or a.audit_type='1004-3') and a.assess_date >= '$begindate' and a.assess_date <= '$enddate' group by a.id ORDER BY a.assess_date DESC";
$res = $db->query($sql);
