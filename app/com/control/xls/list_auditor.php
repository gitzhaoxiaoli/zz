...
if( $cti_code ){
	...
	if( $cti_id ){
		$_pids = array();
		$query = $db->query("SELECT id FROM sp_project WHERE cti_id = $cti_id");
...
if( $aids ){
	...
	while( $rt = $db->fetch_array( $query ) ){
		...
		$resdb[$rt['auditor_id']]['audit_types'][] = read_cache('audit_type', $rt['audit_type'] );
		$resdb[$rt['auditor_id']]['isos'][] = read_cache('iso', $rt['iso'] );
		$resdb[$rt['auditor_id']]['is_leaders'][] = ($rt['is_leader'])?'是':'否';
		$resdb[$rt['auditor_id']]['audit_codes'][] = $rt['audit_code'];
		$resdb[$rt['auditor_id']]['qua_types'][] = read_cache('qualification', $rt['qua_type'] );
	}
}
...
//输出Execl文件
$filename = iconv( 'UTF-8', 'GB2312', '项目派人_').mysql2date( "Y-m-d", nowTime( 'mysql' ) ).".xls";

header("Content-Type: application/vnd.ms-excel");
header("Content-Disposition: attachment; filename=" . $filename );
header("Pragma: no-cache");
header("Expires: 0");
template( 'xls/list_auditor' );
