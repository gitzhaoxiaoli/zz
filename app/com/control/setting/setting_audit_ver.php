...
if( empty( $a ) || 'audit_ver' == $a ) {
	...
	if( $new ){
		...
		foreach( $new['ver'] as $key => $val ){
			...
			$ADDSQL[]		= "( '$iso', '$ver', '$msg', '$audit_basis', '$note', '$vieworder', '$is_stop' )";
		}

		if( $ADDSQL ){
			$add_sql = "INSERT INTO sp_settings_audit_vers ( iso, audit_ver, msg, audit_basis, note, vieworder, is_stop ) VALUES " . implode(',', $ADDSQL );

			$db->query( $add_sql );
		}
	}
 	showmsg('success', 'success', "?m=com&c=setting_audit_ver&a=audit_ver");
}elseif('del'==$a){
