...
//审核开始时间 起
if( $p_start_start ){
	$task_where .= " AND tb_date >= '$p_start_start'";
}
//审核开始时间 止
if( $p_start_end ){
	$task_where .= " AND tb_date <= '$p_start_end'";
}
//审核结束时间 起
if( $p_end_start ){
	$task_where .= " AND te_date >= '$p_end_start'";
}
//审核结束时间 止
if( $p_end_end ){
	$task_where .= " AND te_date <= '$p_end_end'";
}
if( $task_where ){
	$tids = array();
	...
	if( $tids ){
		$where .= " AND cti.tid IN (".implode(',',$tids).")";
	} else {
		$where .= " AND cti.id < -1";
	}
}
...
if( $assess_date_start ){
    $where .= " AND cti.sp_date >= '$assess_date_start'";
}
//评定时间 止
if( $assess_date_end ){
	$where .= " AND cti.sp_date <= '$assess_date_end'";
} 
...
$_eid = current_user("eid");
$where .= " AND cti.eid = '$_eid'";
if( !$export ){
...
$query = $db->query( "SELECT $fields FROM sp_contract_item cti $join WHERE 1 $where ORDER BY cti.cti_id desc $pages[limit]" );
 ...
 while( $rt = $db->fetch_array( $query ) ){
	 $p_info = $db->get_row("SELECT id pid,is_samp,is_check,tid,test_id,is_sample,is_qualified,redata_status,is_finance,pd_type ,assess_date FROM `sp_project` WHERE `cti_id` = '$rt[cti_id]' AND `iso_prod_type` = '1' AND `deleted` = '0' order by id desc limit 1 ");
	 if($p_info){
		 $rt = array_merge($rt,$p_info);
	 }
	 $task_info = $db->get_row("SELECT tb_date,te_date,tb_date,te_date,is_finish,check_result FROM `sp_task` WHERE `id` = '$rt[tid]'");
	 if($task_info){
    ...
    $ep_prod = $db->get_row("select ep_name,audit_type from sp_enterprises where eid=$rt[ep_prod_id]");
