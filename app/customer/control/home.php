...
//if( check_sys_boolen( 'notice' ) ){
	...
	$query = $db->query("SELECT status,COUNT(*) total FROM sp_contract WHERE deleted = '0' AND iso_prod_type = 1 GROUP BY status");
	while( $rt = $db->fetch_array( $query ) ){
	...
	//未安排
	$res=$db->find_one("project"," AND status =0 AND deleted = 0 AND iso_prod_type = 1","COUNT(*)");
	$project_total[0]=$res['COUNT(*)'];
	$query = $db->query("SELECT status,COUNT(*) total FROM sp_task   WHERE 1 AND status IN (1,2,3) and deleted=0 AND iso_prod_type = 1 GROUP BY status");
	while( $rt = $db->fetch_array( $query ) ){
