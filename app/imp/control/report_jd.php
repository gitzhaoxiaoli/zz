...
//$file_arr = showFile(DATA_IMP . "report/计划");
$total_num = array();
$file_name = DATA_IMP . "report_jd.xls";
...
foreach ($data['Sheet1'] as $k => $item) {
    ...
    $cti_id = $db->getField('contract_item','cti_id',array('eid' => $eid));
    // echo $db->sql;
    // echo "<br/>";
    // echo $cti_id;
    // exit();
   ...
   $pid   = $db->getField("project", "id", array(
    ...
    if (!$zsid) {
        ...
        $db->insert("certificate",$new_cert) && $total_num['cert'] ++;
    }
}
// 处理人员
foreach ($data['Sheet2'] as $k => $item) {
    ...
    if(!$cert)continue;
    $p_info = $db->find_one('project',array('cti_id' => $cert['cti_id'] , "audit_type" => '1004-1'),'tid,id');
    $new_tat=array(
            "eid"               => $cert[eid],
            "pid"               => $p_info[id],
            "tid"               => $p_info[tid],
