...
//if( check_sys_boolen( 'notice' ) ){
	...
	//监督维护
	$jd_where[0] = " AND status IN('5','6') AND audit_type IN ('1004-1','1004-2','1004-3') AND deleted = '0'";
	$jd_where[1] = " AND status NOT IN ('5','6') AND audit_type IN ('1004-1','1004-2','1004-3') AND deleted = '0'";
	...
	$query = $db->query("SELECT ifcation_status,COUNT(*) total FROM sp_certificate cert WHERE 1 AND cert.ctfrom >= '01000000' AND cert.ctfrom < '02000000' AND cert.status IN ('01','02','05') AND cert.deleted = '0' AND cert.ifcation_status='0'");
	while( $rt = $db->fetch_array( $query ) ){
		if( $rt['ifcation_status']==0 )
		$ifcation_total[$rt['ifcation_status']] = $rt['total'];
	}
...
$res = $db->find_one("hr",array('id'=>$uid),"openid");
