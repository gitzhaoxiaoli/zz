...
if( $cti_code ){
	$zs_ids=array(-1);
	$query = $db->query("SELECT zs.id FROM sp_contract_item cti left join sp_certificate zs on zs.cti_id=cti.cti_id WHERE cti.cti_code like '%$cti_code%' and cti.deleted=0 and zs.deleted=0  and cti.iso_prod_type = 0");
	while($rt=$db->fetch_array($query)){
		$zs_ids[]=$rt[id];
		}
	$where .= " AND cc.zsid in (".implode(",",$zs_ids).")";
...
$where .= " and cc.deleted='0' and c.iso_prod_type = 0 ";
...
$sql = "SELECT cc.*,c.iso,c.audit_ver,c.cti_id,c.ct_id,c.eid,c.certno,cert_name,cti_code FROM sp_certificate_change cc $join WHERE 1 $where ORDER BY cc.id DESC $pages[limit]";
$query = $db->query( $sql );
while( $rt = $db->fetch_array( $query ) ){
