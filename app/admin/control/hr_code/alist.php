...
$is_hire=getgp('is_hire');
	...
	$hr_where_arr = array();
	 ...
	 $ctfrom = trim(getgp('ctfrom'));
	 $audit_job = trim(getgp('audit_job'));
	 $qua_type = trim(getgp('qua_type'));
	 $iso = trim(getgp('iso'));
	 $is_hire = trim(getgp('is_hire'));
	if( $name ){
	...
	//只显示 审核员 高级审核员 检查员  高级检查员
	// $where .= " AND hq.qua_type IN ('01','02','05','06')";
