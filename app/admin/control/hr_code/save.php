...
$arr_audit = explode('；', $audit_code);
$arr_audit = array_unique($arr_audit); //去除数组中重复值
foreach ($arr_audit as $k => $v) {
    if ($v == '') {
        unset($arr_audit[$k]); //去除空值
     }
};
//验证业务代码
$where = " AND iso = '$iso'";
$where.= " AND code IN ('" . join("','", $arr_code) . "')";
$where.= " AND shangbao IN ('" . join("','", $arr_audit) . "')";
...
//读取人员已有的代码
$sql = "select audit_code from sp_hr_audit_code where qua_id='$qua_id' AND deleted='0' ";
$res = $db->query($sql);
while ($row = $db->fetch_array($res)) {
    $db_info[] = $row['audit_code'];
}; //系统中数据项
$uid = (int)getgp('uid');
...
foreach ($arr_audit as $k=>$p) {
    if (!in_array($p, $db_info) && !empty($p)) { //判断是否已经具有该资质
        $default = array(
            ...
            'iso' => getgp('iso') ,
            'use_code' => $arr_code[$k], //小类代码17A
            'audit_code' => $p, //小类代码17A
            'source' => $source , //能力来源
...
if ($id) { //编辑业务代码
  ...
  $REQUEST_URI = '?c=hr_code&a=edit&uid=' . getgp('uid') . '&iso=' . getgp('iso');
