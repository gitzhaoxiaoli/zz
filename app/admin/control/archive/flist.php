...
if ($_POST) {
    ...
    $f  = $db->get_var("SELECT * FROM sp_project WHERE pd_type > 0 AND id IN (".join("," , $_POST[pid]).")");
    if ($f) {
    	showmsg("有已经评定的项目不能取消分派！" ,"error");
        ...
        $db->update('project', $new_project, array(
            'id' => $_POST[pid]
        ));
        showmsg("success" ,"success" , "?c=archive&a=flist&redata_status=2");
        
    }
    //派人
    if (getgp('type') == 'fenpai') {
        $comment = getgp('comment');
        list($uid, $name) = explode("-", $comment);
        $new_project = array(
            'comment_a_uid' => $uid,
            'comment_a_name' => $name,
            'redata_status' => 1,
        ...
        // p($new_project);
        // exit;
        $uids = $db->get_col("SELECT uid FROM `sp_task_audit_team` WHERE `pid` IN (".join(",",$_POST[pid]).") AND deleted = 0");
        array_push($uids, -1);
        if (in_array($uid, $uids)) {
            showmsg("不能是审核组人员" ,"error");
...
while ($rt = $db->fetch_array($query)) {
    ...
    //证书状态
    $rt['cert_status']  = $db->getField('certificate', 'status', array(
        'eid' => $rt['eid'],
        'iso' => $rt['iso']
    ));
    ...
    if ('0000-00-00 00:00:00' == $rt['upload_done_date'])
        $rt['upload_done_date'] = '';
    $rt[leader] = $db->getField("task_audit_team", "name", array(
        "role" => '1001',
        "tid" => $rt[tid]
    ));
    if (!$redata_status)
...
$hr_arr         = array();
$hr_sql         = "SELECT id,name FROM `sp_hr` WHERE 1  AND deleted = 0 AND job_type LIKE '%1007%' AND is_hire = 1";
$hr_query       = $db->query($hr_sql);
$comment_select = "";
while ($hr_rt = $db->fetch_array($hr_query)) {
    $hr_arr[$hr_rt['id']] = $hr_rt;
    $comment_select .= "<option value=\"$hr_rt[id]-$hr_rt[name]\">$hr_rt[name]</option>";
}
if (!$export) {
    tpl('archive/flist');
} else { //导出excel表格
