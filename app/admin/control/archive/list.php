...
if( $cti_code=trim($cti_code) ){
	...
	if($t_date_s){
		$_tids=$db->get_col("SELECT id FROM `sp_task` WHERE `tb_date` >= '$t_date_s' AND `deleted` = '0'  ORDER BY te_date DESC");
		array_push($_tids, -1);
	...
	if($t_date_e){
		$_tids=$db->get_col("SELECT id FROM `sp_task` WHERE `tb_date` <= '$t_date_e 23:00' AND `deleted` = '0'  ORDER BY te_date DESC");
		array_push($_tids, -1);
		$where .=" AND p.tid in(".implode(",",$_tids).")";
	...
	//要获取的字段
	$fields .= "p.*,t.upload_file_date,t.upload_plan_date,t.bufuhe,t.back_note,p.ctfrom,p.audit_type,p.audit_ver,p.iso,p.redata_note,p.eid,p.tid,p.ct_id";
	...
	if( !$export ){
		...
		$restate_total[2]=$db->get_var("SELECT COUNT(*) total FROM sp_project p $join WHERE 1 $where AND p.redata_status !=0 ");
		/*
	...
	while( $rt = $db->fetch_array( $query ) ){
		...
		$rt['upload_file_date'] = $db->getField('task','upload_file_date',array('id'=>$rt['tid']));
		...
		$rt['back_note'] = $rt['back_note']?$rt['back_note']:'否';
