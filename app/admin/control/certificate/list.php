...
if( !$export and !$export1){
	$total = $db->get_var("SELECT COUNT(*) FROM sp_certificate cert WHERE 1 $where and iso_prod_type = 0");
	$pages = numfpage( $total);
}

$sql = "SELECT cert.* FROM sp_certificate cert $join WHERE 1 $where and iso_prod_type = 0 ORDER BY cert.id DESC $pages[limit]";
