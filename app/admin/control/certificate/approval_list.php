...
$where .= " AND cert.status <> '' and cert.is_check='n' ";
...
if( !$export and !$export1){
	$total = $db->get_var("SELECT COUNT(*) FROM sp_certificate cert WHERE 1 $where");

	$pages = numfpage( $total);
}

$sql = "SELECT cert.* FROM sp_certificate cert $join WHERE 1 $where ORDER BY cert.id DESC $pages[limit]";
