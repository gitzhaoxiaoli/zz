...
if( $cti_code=trim($cti_code) ){ //合同项目编码
	...
	$ne_total = array(
	'1' => $db->get_var("SELECT COUNT(*) total FROM sp_project p $join WHERE 1 $where AND p.ifchangecert = '1' and p.iso_prod_type = 0 "),
	'2' => $db->get_var("SELECT COUNT(*) total FROM sp_project p $join WHERE 1 $where AND p.ifchangecert = '2' and p.iso_prod_type = 0 ")
	);

	$pages = numfpage( $ne_total[$is_check] );
	$where .= "  and p.ifchangecert ='$is_check'";
	$sql = "SELECT p.*,e.ep_name FROM sp_project p $join WHERE 1 $where and p.iso_prod_type = 0  ORDER BY p.id DESC $pages[limit]";
	$query = $db->query( $sql );
	while( $rt = $db->fetch_array( $query ) ){
