...
*/
	...
	//@wangp 限制不显示 已删除的记录
	$where .= " AND cert.deleted = 0 and cert.status in ('01','02')";
	...
	if( !$export ){
		...
		$total = $db->get_var("SELECT COUNT(*) FROM sp_certificate cert $join  where 1 $where and cert.iso_prod_type = 0");
		$pages = numfpage( $total, 20, $url_param );
	}


	//撤销的也不能出现
	$sql = "SELECT cert.*,e.ep_name,ct.ct_code,cti.cti_code FROM sp_certificate cert $join WHERE 1 $where and cert.iso_prod_type = 0 ORDER BY cert.id DESC $pages[limit]";
