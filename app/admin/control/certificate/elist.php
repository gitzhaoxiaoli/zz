...
//证书邮寄列表
	if($_POST){
		if($sms=$_POST[sms]){
			$db->update("sms",array("is_sms"=>'2','sms_no'=>$_POST[sms_no],"sms_date"=>$_POST[sms_date]),array("id"=>$sms));
		}
		showmsg("success","success",'?c=certificate&a=elist');
	...
	$join .= " LEFT JOIN `sp_certificate` c ON c.id = s.temp_id";
	...
	$sql = "SELECT s.*,c.ct_code,c.cti_code,c.iso,c.certno,e.ep_name FROM sp_sms s $join WHERE 1 $where ORDER BY s.id DESC $pages[limit]";
	$query = $db->query( $sql );
	while( $rt = $db->fetch_array( $query ) ){
		$datas[] = $rt;
	}
