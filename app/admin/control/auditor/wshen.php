...
$wenshen_status = (int)getgp("wenshen_status") == 0 ? 1 : (int)getgp("wenshen_status");
...
if( $ct_code ){
	$_tids=array();
	$sql="SELECT tid FROM sp_project WHERE ct_code = '$ct_code' AND deleted = 0 AND iso_prod_type = 0 ";
	$query=$db->query($sql);
...
if( $cti_code ){
	$_tids=array();
	$sql="SELECT tid FROM sp_project WHERE cti_code like '%$cti_code%' and deleted = 0 AND iso_prod_type = 0 ";
	$query=$db->query($sql);
	while( $rt = $db->fetch_array( $query ) ){
		$_tids[]=$rt[tid];
	}
...
if($audit_ver) { //标准版本
	$_tids = array();
	$query = $db->query("SELECT tid FROM sp_project WHERE audit_ver = '$audit_ver' AND deleted = 0 AND iso_prod_type = 0 ");
	while( $rt = $db->fetch_array( $query ) ){
		$_tids[] = $rt['tid'];
	}
	if( $_tids ){
...
$where .= " AND t.deleted = '0'";
	...
	$where .= " AND  t.wenshen_uid = ".current_user('uid')."  AND t.deleted = '0'";
}

$state_total = array(0,0,0);

//计算数量
if( !$export ){
	$query = $db->query("SELECT t.wenshen_status,COUNT(*) total FROM sp_task t  WHERE 1 $where AND t.wenshen_status IN (1,2) AND t.iso_prod_type = 0 GROUP BY t.wenshen_status");
	while( $rt = $db->fetch_array( $query ) ){
...
//审核任务
$tasks = array();
$query = $db->query( "SELECT t.*,e.ep_name,e.areacode,e.person,e.person_tel,e.ep_phone,e.ep_fax,e.ep_level,hr.name FROM sp_task t LEFT JOIN sp_enterprises e ON e.eid = t.eid LEFT JOIN sp_hr hr ON hr.id = t.create_uid WHERE 1 $where AND t.iso_prod_type = 0 ORDER BY t.te_date DESC $pages[limit]" );
...
while( $rt = $db->fetch_array( $query ) ){
	...
	if($export)
		$rt[last_date]=$db->get_var("SELECT te_date FROM `sp_task` WHERE `eid` = '$rt[eid]' AND `deleted` = '0' AND `wenshen_status` = '3' AND `id` < '$rt[id]' and iso_prod_type = 0 ORDER BY `id` DESC ");
...
if( $tasks){
	...
	$sql = "SELECT $fields FROM sp_project p $join WHERE 1 AND p.tid IN (".implode(',',array_keys($tasks)).") AND p.deleted = 0 AND p.iso_prod_type = 0 ORDER BY p.iso " ;
	$query = $db->query( $sql);
	while( $rt = $db->fetch_array( $query ) ){
