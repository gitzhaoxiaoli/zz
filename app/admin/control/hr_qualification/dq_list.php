...
//传入模板数据
	...
	if( !$export ){
		...
		$state_total = array(
		    '0'=>$db->get_var("SELECT COUNT(*) FROM sp_hr_qualification $join WHERE 1 $where AND hq.e_date < '".date("Y-m-d")."'  $is_hire_sql1"),
 			'1'=>$db->get_var("SELECT COUNT(*) FROM sp_hr_qualification $join WHERE 1 $where AND hq.e_date >= '".date("Y-m-d")."'  $is_hire_sql2"),
			);
		$pages = numfpage( $state_total[$status], 20, $url_param );

	}
	if((int)$status){
		$where .= " AND hq.e_date >= '".date("Y-m-d")."'";
	}else{
		$where .= " AND hq.e_date < '".date("Y-m-d")."'";
	}
	$sql = "SELECT *,h.id as uid,hq.id as qid,h.birthday,h.is_hire as is_hire,h.tel,h.unit,h.areacode_str,
			...
			FROM sp_hr_qualification $join WHERE 1 $where ORDER BY hq.id DESC $pages[limit]" ;
	
	$query = $db->query($sql);
	$date=date("Y-m-d H:i:s");
	while( $rt = $db->fetch_array( $query ) ){
		$two_year=thedate_add($rt[s_date],2,"year");
		$one_year=thedate_add($rt[s_date],1,"year");
		if(strtotime($date)>=strtotime($two_year)){
			$date_b=$two_year;
			$date_e=$rt[e_date]." 00:00:00";
			
		}elseif(strtotime($two_year)>strtotime($date) and strtotime($date)>=strtotime($one_year)){
			$date_b=$one_year;
			$date_e=$two_year;
		}elseif(strtotime($one_year)>strtotime($date)){
			$date_b=$rt[s_date]." 00:00:00";
			$date_e=$one_year;
		
		}
		$rt['audit_times'] = $db->get_var("select COUNT(*) from sp_task_audit_team where uid = {$rt[uid]} and iso='{$rt[iso]}' and taskBeginDate >= '{$date_b}' and taskEndDate <= '{$date_e}' and deleted=0");
		$rt['leader_times'] = $db->get_var("select COUNT(*) from sp_task_audit_team where uid = {$rt[uid]} and role='1001' and iso='{$rt[iso]}' and taskBeginDate >= '{$date_b}' and taskEndDate <= '{$date_e}' and deleted=0");
		...
		if ($export and $rt[uid]) {
			...
			$hr_info=$db->get_row("SELECT department,position FROM sp_hr_experience  where deleted='0' AND add_hr_id=$rt[uid] and type='j'");
