...
if($year=trim(getgp("year"))){
	...
	$query=$db->query("SELECT p.*,e.ep_name,t.tb_date,t.te_date,t.save_date,t.rect_date FROM `sp_project` p LEFT JOIN sp_enterprises e ON e.eid=p.eid LEFT JOIN sp_task t ON t.id=p.tid   WHERE p.`deleted` = '0' and p.iso_prod_type = 0 AND `tid` IN (".join(",",$tids).") order by t.te_date");
	$data=array();
	while($rt=$db->fetch_array($query)){
		...
		if($rt[audit_type]=='1004-1' or $rt[audit_type]=='1004-2')
			$arr[4]=f_audit_type($rt[audit_type]);
		if($rt[audit_type]=='1007')
			$arr[5]="√";
		$arr[6]=$iso_arr[$rt['iso']];
		$arr[7]=$db->get_var("SELECT name FROM `sp_task_audit_team` WHERE `tid` = '$rt[tid]' AND `deleted` = '0' and iso_prod_type = 0 AND `role` = '1001'");
		$auditor=$db->get_col("SELECT name FROM `sp_task_audit_team` WHERE `tid` = '$rt[tid]' AND `deleted` = '0' and iso_prod_type = 0 AND `role` <> '1001'");
		$arr[8]=join(",",$auditor);
