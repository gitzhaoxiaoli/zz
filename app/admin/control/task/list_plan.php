...
//计算数量
if( !$export ){
	$query = $db->query("SELECT t.jh_sp_status,COUNT(*) total FROM sp_task t  WHERE 1 $where and t.iso_prod_type = 0 AND t.jh_sp_status IN (0,1) GROUP BY t.jh_sp_status");
	while( $rt = $db->fetch_array( $query ) ){
...
//审核任务 主表
$tasks = array();
$query = $db->query( "SELECT t.*,e.ep_name,hr.name FROM sp_task t LEFT JOIN sp_enterprises e ON e.eid = t.eid LEFT JOIN sp_hr hr ON hr.id = t.create_uid WHERE 1 $where and t.iso_prod_type = 0 ORDER BY t.id DESC $pages[limit]" );
// echo $db->sql;
while( $rt = $db->fetch_array( $query ) ){
	...
	$rt['leader']=$db->get_var("SELECT name FROM `sp_task_audit_team` WHERE `tid` = '$rt[id]' AND `deleted` = '0' and iso_prod_type = 0 AND `role` = '1001' ");
...
if( $tasks ){
	...
	$sql = "SELECT $fields FROM sp_project p $join WHERE 1 AND p.tid IN (".implode(',',array_keys($tasks)).") AND p.deleted = 0 and p.iso_prod_type = 0" ;
	$query = $db->query( $sql);
	while( $rt = $db->fetch_array( $query ) ){
