...
$is_hire = isset($is_hire) ? $is_hire : 2;
...
if ($job_type) {
	...
	$job_type_select = str_replace("value='$job_type'>", "value='$job_type' selected>", $job_type_select);
}
...
$where.= " AND sp_hr.deleted = 0 AND is_hire = '1'";
    ...
    $query = $db->query("SELECT sp_hr.wstatus,COUNT(*) total FROM sp_hr $join WHERE 1 $where GROUP by sp_hr.wstatus");
    while ($rt = $db->fetch_array($query)) {
        $hire_total[$rt['wstatus']] = $rt['total'];
    }
if (!$export and $is_hire == 0) {
    
    $pages = numfpage($hire_total[$is_hire]);
}
$where.= " AND wstatus = '$is_hire' ";
