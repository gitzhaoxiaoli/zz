...
$ct_id = (int)getgp( 'ct_id' );
	if( $ct_id ){
		...
		$sql = "SELECT $fields FROM sp_project p $join WHERE p.ct_id = '$ct_id' AND p.deleted=0 and t.deleted=0 and cti.iso_prod_type = 0 and p.iso_prod_type = 0 and t.iso_prod_type = 0 order by t.te_date DESC";
		...
		while( $rt = $db->fetch_array( $query ) ){
			...
			*/
			...
			if($rt[tid]){
				$wheres = "and  tid='$rt[tid]' and deleted=0";
				$res=$db->find_results("task_audit_team",$wheres);
			...
			unset($cert_info);
			if($rt[zsid])
				$rt[change]=$db->get_results("select cg_type,cgs_date,cge_date,certpasue_value2,cg_reason,cg_af,cg_bf,note from sp_certificate_change where 1 AND zsid='$rt[zsid]'");
			//p($rt[change]);
			$rt=chk_arr($rt);
