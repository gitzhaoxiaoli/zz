...
if( $step ){
    ...
    $exc_psrs = getgp( 'exc_psrs' );//
    $exc_com = getgp( 'exc_com' );//
	...
	$combine_note = getgp( 'combine_note' );
	$note = getgp( 'note' );
	$mark = getgp( 'mark' );
	...
	if( $audit_codes ){
		foreach( $audit_codes as $cti_id => $audit_code ){
			...
			$cti_array = array(
				...
				'exc_psrs'		=> $exc_psrs[$cti_id], //
				'exc_com'		=> $exc_com[$cti_id], //
	...
	$new_ct = array(
		...
		'review_user' => current_user( 'name' ),
		'status' => $status,
		'major_person'	=>$zy_name //项目管理人员
	);
	...
	//是否禁用保存按钮
	$disabled_save = ( $status = 1 or $status = 4 ) ? '' : 'disabled';
	// p($disabled_save);
	if( $status >= 2 ){
		$checked = ' checked';
	} else {
 ...
 */		$rt['audit_ver_V'] = f_audit_ver( $rt['audit_ver'] );
	...
	if( $_audit_codes ){
		foreach( $_audit_codes as $cti_id => $codes ){
			$query = $db->query("SELECT code,shangbao,risk_level,mark FROM sp_settings_audit_code WHERE iso = '{$codes['iso']}' AND  shangbao IN('".implode("','",$codes[audit_code])."')");
			while( $rt = $db->fetch_array( $query ) ){
			    ...
			    $_audit_code[$cti_id][] = $rt['shangbao'];
			    $_use_code[$cti_id][] = $rt['code'];
			}
			$ct_items[$cti_id]['audit_code'] = join("；",$_audit_code[$cti_id]);
			$ct_items[$cti_id]['use_code'] = join("；",$_use_code[$cti_id]);
		}

	}
	// p($_audit_codes);die;
	$wheres = "AND `eid` = '$c_info[eid]' AND `ct_id` <> '$ct_id' AND `deleted` = '0' AND iso_prod_type = 0 ";
	$field = "ct_id";
	$ct_id_f=$db->find_one("contract",$wheres,$field);
    
    $is_125 = $db->meta($eid,"is_125","","enterprise");
...
///	!$ct_ps_name && $ct_ps_name=current_user("name");
	...
	//项目管理人员信息
	$sql="select * from sp_hr where job_type=1001 and deleted=0";
	$h=$db->query($sql);
