...
if( $audit_code ){
    $where .= " AND p.audit_code LIKE '%$audit_code%'";
...
while( $rt = $db->fetch_array( $query ) ){
	...
	if(!$rt['witness']){
		$rt['witness']="";
		$_row=$db->get_row("SELECT * FROM `sp_task_audit_team` WHERE `tid` = '$rt[tid]' AND `deleted` = '0' AND iso='$rt[iso]' AND `witness` <> '' AND `witness_person` like '%".f_username($rt[uid])."%'");
    ...
    $sql1  = "select pid,uid,name,role,use_code,qua_type,iso_prod_type from sp_task_audit_team where tid='{$rt[tid]}' and deleted=0 and iso='$rt[iso]'";
