...
//要获取的字段
$fields.= "p.*,e.ep_name,e.ctfrom,e.areacode,e.ep_level,e.person,e.ep_phone";
//要关联的表
...
while ($rt = $db->fetch_array($query)) {
	...
	if($export=='2'){
		...
		$shsj=$t_info[tb_date]."至".$t_info[te_date];
		$e_info=$enterprise->get(array("eid"=>$rt[eid]));
		$content=$e_info[person].$e_info[ep_phone]."\n".$e_info[person_tel]."\n".$e_info[person_mail];
		$data[]=array($rt[ep_name],$rt[use_code],$rt[audit_code],"","",f_iso($rt[iso])."-".$rt['audit_type_V'],"",$shsj,"",$content);
	}
