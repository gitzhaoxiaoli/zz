...
$where .= " AND p.deleted = '0' AND p.iso_prod_type = 0";
if (!$export) {
...
while ($rt = $db->fetch_array($query)) {
	...
	$province = substr($rt['areacode'],0, 2) . '0000';
    $rt['province']     = f_region_province($province);
    if ($rt[audit_require])
        $rt[style] = 'style="background-color:#ffddde;"';
	if($rt[audit_type]=='1007'){
	    ...
	    $rt['cert_status'] = $db->get_var("SELECT status FROM `sp_certificate` WHERE `eid` = '$rt[eid]' AND `deleted` = '0' AND `cti_id` <> '$rt[cti_id]' ORDER BY `e_date` DESC ");
		...
		$rt['cert_status']=$db->get_var("SELECT status FROM `sp_certificate` WHERE  `deleted` = '0' AND `cti_id` ='$rt[cti_id]' ORDER BY `e_date` DESC ");
        $rt['cert_status'] =read_cache('certstate',$rt['cert_status']);
	}
