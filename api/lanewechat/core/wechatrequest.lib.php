...
class WechatRequest{
    ...
    public static function text(&$request){
        $queryUrl = "http://www.tuling123.com/openapi/api";
        $param = [
            'key' => 'a77cce0b241a4ba2a021101e68c3b487',
            'info' => $request['content']
            ];
        $res = Curl::callWebServer($queryUrl, $param);
        $content = $res['text'];
        // $content = '收到文本消息11' . $request['content'];
    ...
    public static function eventSubscribe(&$request){
        require_once 'api/mmysql.class.php';
        $conf = require 'data/db_config.php';
        $mysql = new Mmysql($conf);
        $uid = $request['eventkey'];
        $mysql->where(array('id'=>$uid))->update('sp_hr',array('openid' => $request['fromusername'])) ;       
        $content = '欢迎您关注';
        return ResponsePassive::text($request['fromusername'], $request['tousername'], $content);
    }
