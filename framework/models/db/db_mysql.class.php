...
class db_mysql
{
    ...
    */
    ...
    {
		if(!is_array($where_arr) or !$where_arr)return;
    ...
    public function find_results($table='', $where = '', $fields = '*', $pages=array())
    {
        if (is_array($where)) {
    ...
    function insert($table, $args, $is_default=true)
    {
		...
		if($_SESSION[appname] == 'en_login'){
			$default[create_user] = current_user("ep_name");
		}
		if($is_default)
    ...
    function meta($id, $meta_name = '', $meta_value = '', $used = '')
    {
        ...
        if ($meta_name && $meta_value) {
            ...
            $result = $this->get_var("SELECT meta_value FROM sp_$table WHERE ID = '$id' AND meta_name = '$meta_name' AND used = '$used'");
        } else {
            $result = array();
            $query  = $this->query("SELECT * FROM sp_$table WHERE ID = '$id' AND used = '$used'");
            while ($rt = $this->fetch_array($query)) {
