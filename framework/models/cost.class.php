...
class cost{
	...
	function get( $id ){
		global $db;
		$row = $db->get_row("SELECT * FROM sp_contract_cost WHERE id = '$id' and deleted='0'");
		return $row;
	}

	function del( $id ){
		global $db;
		$row = $db->query("update sp_contract_cost set deleted='1' WHERE id = '$id'");
		return $row;
	}
	...
	function gets( $ids ){
		...
		global $db;
		$result = array();
		$query = $db->query("SELECT * FROM sp_contract_cost WHERE ct_id =$ids and deleted=0");
